...
from split_residual import Model
...
def train_fn(Upmodel,Train_loader,Val_loader,opt,mse,log_folder,log_file_name,epoch):
...
#1.489
        ...
        if idx % 500 == 0:
            ...
            new_line=f'mse:{"%.3f"%mse_loss.item()}' + '\n'#; Cycle loss:{"%.3f" % cycle_loss.item()}'# ,perc loss:{"%.3f" % perc_loss.item()}' #,fwd_loss:{"%.3f"%cyc_fwd.item()},bwd_loss:{"%.3f"%cyc_bwd.item()} '
    ...
    log_file = open(log_file_name.split('.')[0] + "_train_val_loss", "+a")
...
def main():
    Upmodel=Model().to(config.DEVICE)
    #grl=GRL().to(config.DEVICE)
    #Downmodel=DownGenerator().to(config.DEVICE)
    opt = optim.Adam(list(Upmodel.parameters()), lr=config.LEARNING_RATE,   #list(Downmodel.parameters())+list(grl.parameters())+
...
#    os.mkdir(log_folder + '/checkpoint')
    ...
    loss_file.close()
    loss_file = open(log_file_name.split('.')[0] + "_train_val_loss", "w")
    loss_file.write('Starting Training\n')
    loss_file.close()
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        train_fn(Upmodel,Train_loader,Val_loader,opt,mse,log_folder,log_file_name,epoch)

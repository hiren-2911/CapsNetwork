...
def preprocess_one_image(image_path: str, upscale_factor: int, device: torch.device) -> [Tensor, ndarray, ndarray]:
    image = cv2.imread(image_path).astype(np.float32) / 255.0
    bic_image = image_resize(image, upscale_factor)

    # Get Y channel image data
    y_image = bgr_to_ycbcr(image, True)

    # Get Cb Cr image data from hr image
    bic_ycbcr_image = bgr_to_ycbcr(bic_image, False)
    _, bic_cb_image, bic_cr_image = cv2.split(bic_ycbcr_image)
    ...
    return y_tensor, bic_cb_image, bic_cr_image
...
def test():
    ...
    output=rgb_to_ycbcr_torch(x,True)
    print(output.shape)
    #print(output.shape)
